...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            u1_des = u + int(ORB_pattern[i*4]*cos(theta)-ORB_pattern[i*4+1]*sin(theta));
            v1_des = v + int(ORB_pattern[i*4]*sin(theta)+ORB_pattern[i*4+1]*cos(theta));
            u2_des = u + int(ORB_pattern[i*4+2]*cos(theta)-ORB_pattern[i*4+3]*sin(theta));
            v2_des = v + int(ORB_pattern[i*4+2]*sin(theta)+ORB_pattern[i*4+3]*cos(theta));
            if(!(u1_des>0&&u1_des<640&&v1_des>0&&v1_des<640&&u2_des>0&&u2_des<640&&v2_des>0&&v2_des<640))
            {
                d.clear();
                break;//这一组的orb特征描述符不完整 跳过该点
            }
            else {
                double intensity1 = image.at<uchar>(v1_des, u1_des);
                double intensity2 = image.at<uchar>(v2_des, u2_des);
...
//            if (d2.empty()) continue;
        ...
        minIdx = distance(begin(Distance),min_element(begin(Distance), end(Distance)));
        if (Distance[minIdx] > 0 && Distance[minIdx]< d_max){
            cv::DMatch match;
            match.queryIdx = i;
            match.trainIdx = minIdx;
            match.distance = Distance[minIdx];
