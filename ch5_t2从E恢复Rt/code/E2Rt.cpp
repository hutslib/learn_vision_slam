...
int main(int argc, char **argv) {
    ...
    Matrix3d S = U.inverse()*E*V.inverse();
    vector<double> sigma = {S(0,0),S(1,1),S(2,2)};
    sort(sigma.begin(),sigma.end());
    S << (sigma.at(2)+sigma.at(1))/2, 0, 0,
    ...
    t_wedge1 = U*R_z1*S*U.transpose();
    R1 = U*R_z1*V.transpose();
    t_wedge2 = U*R_z2*S*U.transpose();
